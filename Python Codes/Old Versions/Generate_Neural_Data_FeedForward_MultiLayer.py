...
input_opts, args = getopt.getopt(sys.argv[1:],"hE:I:P:Q:T:S:B:D:A:F:R:L:")
if (input_opts):
    for opt, arg in input_opts:
        if opt == '-E':
            NeuralNet.n_exc_array = np.matrix(str(arg))                 # The number of excitatory neurons in each layer
        elif opt == '-I':
            NeuralNet.n_inh_array = np.matrix(str(arg))                 # The number of excitatory neurons in each layer            
        elif opt == '-P':
            NeuralNet.connection_prob_matrix = np.matrix(str(arg))      # The probability of having a link from each layer to the other. Separate the rows with a ";"
        elif opt == '-Q':
