...
input_opts, args = getopt.getopt(sys.argv[1:],"hN:Q:T:S:D:A:F:R:L:M:B:X:Y:C:V:H:J:U:Z:b:p:j:o:f:")
...
#kernel_choice = 'E'
no_avg_itr = 4                                 # Number of times the inference algorithm is run with different parameters to find the connectivtiy
...
for n_ind in neuron_range:
    ...
    if no_structural_connections:
        ...
        zero_connections_orig = list(zero_connections_orig)
        ...
        zero_connections_orig = np.array(zero_connections_orig)
    ...
    if len(hidden_neurons):
        hidden_neurons_new  = [item for item, count in Counter(hidden_neurons).items() if count == 1]
    ...
    W_inferred = np.reshape(W_inferred,[len(W_inferred),1])
