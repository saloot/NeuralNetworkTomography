function BP_rule_leaky_inhibitory(n_exc,n_inh,T,tau,theta,p,q,no_averaging_itrs,delta_plus,delta_minus,B_LLR_flag,mode,weight_rule,Delta,epsilon_plus,epsilon_minus,binary_mode)
...
for av_itr = 1:no_averaging_itrs        
    ...
    if (mode == 4)
        for i = 1:n
            ...
            g = g';
            R = S(:,i);
            [W,W2]= infer_connection(n_exc,n_inh,S,R,T,weight_rule,p,tau,theta,B_LLR_flag,delta_plus,delta_minus,Delta);
            deg_max = round(2*p*n);
            aa = sum(sum(S))/T/n;
            r = sum(R)/T;
